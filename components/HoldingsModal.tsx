    ...
    >
      <div className="modal-panel bg-white w-full sm:max-w-lg sm:rounded-2xl rounded-t-3xl shadow-2xl flex flex-col max-h-[92vh] sm:max-h-[88vh] overflow-hidden">
        ...
        <div className="px-5 pt-3 sm:pt-5 pb-4 border-b border-slate-100 shrink-0">
          ...
          {/* Return + momentum + streak + breadth */}
          <div className="flex items-center gap-2 mt-3 flex-wrap">
            <span className={`font-mono text-2xl font-bold tabular-nums ${pctClass}`}>
              {sectorVal !== null
            ...
            {!loading && holdings.length > 0 && (
              <span className={`text-xs ${streak >= 5 ? '' : 'ml-auto'}`}>
                ...
                {breadth !== null && (
                  <span className="text-slate-400"> · {breadth}% breadth</span>
                )}
              </span>
            )}
          </div>
              ...
              <div className="space-y-0.5">
                {sorted.map((h, i) => {
                  ...
                  return (
                    ...
                    >
                      <span className="text-xs text-slate-300 w-5 text-right shrink-0 tabular-nums">
                        {i + 1}
                      </span>
                      ...
                      <span className="font-mono text-xs text-slate-400 w-16 text-right shrink-0 tabular-nums">
                        {formatPrice(h.price)}
                      </span>
        ...
        <div className="px-5 py-3 border-t border-slate-100 shrink-0 flex items-center justify-between">
          <p className="text-[10px] text-slate-400">
            Eagleview v4.0 · Yahoo Finance · equal-weighted
          </p>
...
function BenchmarkComparison({ sectorVal, sectorName, benchmarks, period }: {
  ...
  const Row = ({
    ...
    return (
      <div className={`flex items-center gap-2 py-1.5 ${bold ? 'font-semibold' : ''}`}>
        <span className={`text-xs w-28 shrink-0 truncate ${bold ? 'text-slate-800' : 'text-slate-500'}`}>
          {label}
        </span>
  ...
  return (
    <div>
      ...
      <Row
        label={`▸ ${sectorName.length > 16 ? sectorName.slice(0, 15) + '…' : sectorName}`}
        val={sectorVal}
        bold
      />
      ...
      <p className="text-[9px] text-slate-300 mt-2">
        Alpha = sector minus benchmark · {period}
      </p>
    </div>
  )
}
  ...
  return (
    <div className="text-center py-10">
      ...
      <p className="text-slate-400 text-xs mt-1">
        Run the data sync to populate stocks.
      </p>
    </div>
  )
}
